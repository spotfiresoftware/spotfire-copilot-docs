# Spotfire License MCP Server Deployment Guide
...
This guide explains how to deploy the Spotfire License MCP server (`spotfire-lic`) from OCI artifacts.
...
- `SFLIC_MCP_SERVER_URL=http(s)://<host>/mcp`
...
- Container image: `copilotoci.azurecr.io/spotfirecopilot/mcp-spotfire-lic:<image-tag>`
- Helm chart: `oci://copilotoci.azurecr.io/spotfirecopilot/mcp-spotfire-lic`
...
```bash
helm show chart oci://copilotoci.azurecr.io/spotfirecopilot/mcp-spotfire-lic \
  --version <chart-version>

docker pull copilotoci.azurecr.io/spotfirecopilot/mcp-spotfire-lic:<image-tag>
```
...
```env
MCP_IMAGE_REF=copilotoci.azurecr.io/spotfirecopilot/mcp-spotfire-lic:<image-tag>
HOST=0.0.0.0
PORT=8064
TRANSPORT=streamable-http
...
|---|---|---|---|
| MCP_IMAGE_REF | Yes | OCI image reference for this server. | copilotoci.azurecr.io/spotfirecopilot/mcp-spotfire-lic:1.0.0 |
| HOST | Yes | Server bind address. | 0.0.0.0 |
| PORT | Yes | Server listening port. | 8064 |
...
| SF_CLIENT_ID | Yes | OAuth client ID for Spotfire API access. | spotfire-lic-client |
...
| MCP_AUTH_AUDIENCE | No | Optional JWT audience claim (`aud`) to enforce. | mcp-spotfire-lic |
...
image:
  ...
  repository: spotfirecopilot/mcp-spotfire-lic
  tag: "<image-tag>"
...
```bash
helm upgrade --install mcp-spotfire-lic oci://copilotoci.azurecr.io/spotfirecopilot/mcp-spotfire-lic \
  --namespace mcp-spotfire-lic \
  --create-namespace \
  -f values.yaml
```
...
```bash
docker compose ps
docker compose logs mcp-spotfire-lic --tail=200
curl -fsS http://localhost:8064/healthz
curl -fsS http://localhost:8064/readyz
```

### 5.2 Kubernetes

```bash
kubectl -n mcp-spotfire-lic get pods
kubectl -n mcp-spotfire-lic get svc
kubectl -n mcp-spotfire-lic logs deploy/mcp-spotfire-lic --tail=200
...
```bash
export MCP_IMAGE_REF=copilotoci.azurecr.io/spotfirecopilot/mcp-spotfire-lic:<new-image-tag>
docker compose pull mcp-spotfire-lic
docker compose up -d
```
...
```bash
helm upgrade mcp-spotfire-lic oci://copilotoci.azurecr.io/spotfirecopilot/mcp-spotfire-lic -n mcp-spotfire-lic -f values.yaml
```
...
```bash
helm uninstall mcp-spotfire-lic -n mcp-spotfire-lic
```

## 8. Troubleshooting

1. Spotfire auth failures:
- Validate Spotfire OAuth credentials and admin permissions.
2. Mutation calls denied:
- Confirm identity permissions for license/group management operations.
3. Transport mismatch:
- Ensure clients target `/mcp` to match deployment mode.

## 9. Related Documentation

See this community site's Spotfire License MCP Server overview and tools reference for mutating vs read-only tool behavior.
