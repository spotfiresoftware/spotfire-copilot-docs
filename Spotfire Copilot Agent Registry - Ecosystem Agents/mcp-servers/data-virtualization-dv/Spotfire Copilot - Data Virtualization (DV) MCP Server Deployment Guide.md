# Data Virtualization (DV) MCP Server Deployment Guide
...
This guide explains how to deploy the DV MCP server from OCI artifacts and validate connectivity to the Data Virtualization backend.
...
- `DV_MCP_SERVER_URL=http(s)://<host>/mcp`
...
- Container image: `copilotoci.azurecr.io/spotfirecopilot/mcp-dv:<image-tag>`
- Helm chart: `oci://copilotoci.azurecr.io/spotfirecopilot/mcp-dv`
...
```bash
helm show chart oci://copilotoci.azurecr.io/spotfirecopilot/mcp-dv \
  --version <chart-version>

docker pull copilotoci.azurecr.io/spotfirecopilot/mcp-dv:<image-tag>
```
...
```env
MCP_IMAGE_REF=copilotoci.azurecr.io/spotfirecopilot/mcp-dv:<image-tag>
HOST=0.0.0.0
PORT=8060
TRANSPORT=streamable-http

DV_URL=https://<dv-host>
...
|---|---|---|---|
| MCP_IMAGE_REF | Yes | OCI image reference for this server. | copilotoci.azurecr.io/spotfirecopilot/mcp-dv:1.0.0 |
...
| DV_URL | Yes | Data Virtualization server base URL. | https://dv.example.com |
| DV_USERNAME | Yes | Data Virtualization service username. | admin |
| DV_PASSWORD | Yes | Data Virtualization service password. | replace-me |
...
| MCP_AUTH_AUDIENCE | No | Optional JWT audience claim (`aud`) to enforce. | mcp-dv |
...
image:
  ...
  repository: spotfirecopilot/mcp-dv
  tag: "<image-tag>"
...
```bash
helm upgrade --install mcp-dv oci://copilotoci.azurecr.io/spotfirecopilot/mcp-dv \
  --namespace mcp-dv \
  --create-namespace \
  -f values.yaml
```
...
```bash
docker compose ps
docker compose logs mcp-dv --tail=200
...
```bash
kubectl -n mcp-dv get pods
kubectl -n mcp-dv get svc
kubectl -n mcp-dv logs deploy/mcp-dv --tail=200
...
```bash
export MCP_IMAGE_REF=copilotoci.azurecr.io/spotfirecopilot/mcp-dv:<new-image-tag>
docker compose pull mcp-dv
docker compose up -d
```
...
```bash
helm upgrade mcp-dv oci://copilotoci.azurecr.io/spotfirecopilot/mcp-dv -n mcp-dv -f values.yaml
```
...
```bash
helm uninstall mcp-dv -n mcp-dv
```

## 8. Troubleshooting

1. Backend auth failures:
- Validate `DV_USERNAME` and `DV_PASSWORD`.
2. Query failures:
- Verify `DV_URL` and reachable OData/catalog endpoints.
3. Transport mismatch:
- Confirm client URL matches deployed transport (`/mcp`).

## 9. Related Documentation

See this community site's DV MCP Server overview and tools reference for tool behavior and request/response expectations.
