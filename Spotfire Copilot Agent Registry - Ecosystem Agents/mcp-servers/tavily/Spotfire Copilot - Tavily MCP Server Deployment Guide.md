# Tavily MCP Server Deployment Guide
...
This guide explains how to deploy the Tavily MCP server from OCI artifacts.
...
- `TAVILY_API_KEY`
- `TAVILY_READINESS_MODE` (`config_only`, `strict_external`, `external_degraded_ok`)
- `TRANSPORT` (`streamable-http`)
- `HOST`, `PORT`

Client-side connection setting:

- `TAVILY_MCP_SERVER_URL=http(s)://<host>/mcp`
...
- Container image: `copilotoci.azurecr.io/spotfirecopilot/mcp-tavily:<image-tag>`
- Helm chart: `oci://copilotoci.azurecr.io/spotfirecopilot/mcp-tavily`
...
```bash
helm show chart oci://copilotoci.azurecr.io/spotfirecopilot/mcp-tavily \
  --version <chart-version>

docker pull copilotoci.azurecr.io/spotfirecopilot/mcp-tavily:<image-tag>
```
...
```env
MCP_IMAGE_REF=copilotoci.azurecr.io/spotfirecopilot/mcp-tavily:<image-tag>
HOST=0.0.0.0
PORT=8058
TRANSPORT=streamable-http

TAVILY_API_KEY=<tavily-api-key>
TAVILY_READINESS_MODE=config_only
```
...
|---|---|---|---|
| MCP_IMAGE_REF | Yes | OCI image reference for this server. | copilotoci.azurecr.io/spotfirecopilot/mcp-tavily:1.0.0 |
| HOST | Yes | Server bind address. | 0.0.0.0 |
| PORT | Yes | Server listening port. | 8058 |
| TRANSPORT | Yes | MCP transport mode. Use `streamable-http` in this guide. | streamable-http |
| TAVILY_API_KEY | Yes | Tavily API key for search, answer, and news tools. | tvly-... |
| TAVILY_READINESS_MODE | No | Tavily readiness mode for external dependency checks. | config_only |
...
| MCP_AUTH_AUDIENCE | No | Optional JWT audience claim (`aud`) to enforce. | mcp-tavily |
...
image:
  ...
  repository: spotfirecopilot/mcp-tavily
  tag: "<image-tag>"
...
```bash
helm upgrade --install mcp-tavily oci://copilotoci.azurecr.io/spotfirecopilot/mcp-tavily \
...
```bash
docker compose ps
docker compose logs mcp-tavily --tail=200
curl -fsS http://localhost:8058/healthz
curl -fsS http://localhost:8058/readyz
```

### 5.2 Kubernetes

```bash
kubectl -n mcp-tavily get pods
kubectl -n mcp-tavily get svc
kubectl -n mcp-tavily logs deploy/mcp-tavily --tail=200
...
```bash
export MCP_IMAGE_REF=copilotoci.azurecr.io/spotfirecopilot/mcp-tavily:<new-image-tag>
docker compose pull mcp-tavily
docker compose up -d
```
...
```bash
helm upgrade mcp-tavily oci://copilotoci.azurecr.io/spotfirecopilot/mcp-tavily -n mcp-tavily -f values.yaml
```
...
```bash
helm uninstall mcp-tavily -n mcp-tavily
```

## 8. Troubleshooting

1. `readyz` behavior unexpected:
- Confirm `TAVILY_READINESS_MODE` is set for your operational preference.
2. Search calls fail:
- Validate `TAVILY_API_KEY` and external connectivity to Tavily APIs.
3. Quota concerns:
- Use `config_only` readiness mode to avoid frequent external probe calls.

## 9. Related Documentation

See this community site's Tavily MCP Server overview and tools reference for search/answer/news parameters and output format.
